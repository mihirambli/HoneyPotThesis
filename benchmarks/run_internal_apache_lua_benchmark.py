...
Writes machine-readable results to:
  benchmarks/results/internal_apache_lua_profile.json
"""
...
def cycle_loadtester(env: dict[str, str]) -> tuple[subprocess.CompletedProcess[str], datetime]:
    ...
    The apache and backend containers are left running so Apache state is preserved between
    VU levels. Returns the wait result and a timestamp captured just before the container
    started, used with --since to isolate this run's Apache log lines from previous runs.
    """
    run_cmd(
