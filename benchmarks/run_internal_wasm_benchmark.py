...
Writes machine-readable results to:
  benchmarks/results/internal_wasm_profile.json        (summary stats, microseconds)
  benchmarks/results/internal_wasm_raw.json            (raw per-request samples, microseconds)
  benchmarks/results/e2e_wasm_wadm.json                (end-to-end k6 latency, milliseconds)
"""
...
# directly comparable.
E2E_EDGE = "wasm"
E2E_EDGE_CONFIG = "envoy-wasm/envoy-wasm.yaml"
...
def start_wasm_stack(env: dict[str, str]) -> subprocess.CompletedProcess[str]:
    ...
    `--build` is required: without it Compose reuses any existing rust-builder image, so an edit
    to wasm-filter/ would silently be benchmarked against the previously compiled filter.wasm.
    """
    return run_cmd(
        ["docker", "compose", "--profile", "wasm", "up", "-d", "--build"],
        env=env,
    )
...
def main() -> int:
    ...
    e2e_output_file = results_dir / "e2e_wasm_wadm.json"
